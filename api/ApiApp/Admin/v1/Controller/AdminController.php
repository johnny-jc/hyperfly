...
class AdminController extends BaseController
{
    ...
    public function login()
    {
        ...
        return $login ?
            ...
            $this->fail($this->adminModel->getModelError());
    }
    ...
    {
        return $this->adminModel->logoutByAdminAccessToken($this->getAdminAccessToken()) ?
            $this->success() :
            $this->fail($this->adminModel->getModelError());
    }
    ...
    public function createAdmin()
    {
        $rules = [
            'username' => [
                ...
                'min:4',
                'max:16',
                'unique:' . $this->adminModel->getTable() . ',username',
            ],
            'password' => [
                'required',
                'regex:/^[0-9a-zA-Z_]*$/',
            ...
            'status' => [
                'required',
                'integer',
                'in:' . AdminModel::ADMIN_STATUS_ENABLE . ',' . AdminModel::ADMIN_STATUS_DISABLE
            ],
        ];
        $message = [
            //username start
        ...
        return $this->adminModel->createAdmin($createData) ?
            $this->success() :
            $this->fail();
    }
    ...
    public function updateAdminStatusById()
    {
        $rules = [
            ...
            'status' => [
                'required',
                'integer',
                'in:' . AdminModel::ADMIN_STATUS_DISABLE . ',' . AdminModel::ADMIN_STATUS_ENABLE
            ],
        ];
        $message = [
            ...
            'status.required' => '参数:attribute不存在',
            'status.integer' => '参数:attribute的值只能是数字',
            'status.in' => '参数:attribute的值只能是' . AdminModel::ADMIN_STATUS_ENABLE . '或' . AdminModel::ADMIN_STATUS_DISABLE,
        ];
        $validate = $this->validator($this->request->all(), $rules, $message);
        if ($validate->fails()) {
            return $this->fail($validate->errors()->first());
        ...
        return $this->adminModel->updateAdminStatus($condition, $updateData) ?
            $this->success() :
            $this->fail($this->adminModel->getModelError());
    }
    ...
    public function deleteAdminById()
    {
        ...
        return $this->adminModel->deleteAdminById((int)$this->request->input('id')) ?
            $this->success() :
            $this->fail($this->adminModel->getModelError());
    }
    ...
    public function updateAdminById()
    {
        $rules = [
            ...
            'username' => [
                ...
                'min:4',
                'max:16',
                Rule::unique($this->adminModel->getTable())->ignore($this->request->input('id')),
            ],
            'password' => [
                'regex:/^[0-9a-zA-Z_]*$/',
                'min:6',

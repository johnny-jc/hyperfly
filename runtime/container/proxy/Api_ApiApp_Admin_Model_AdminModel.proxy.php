...
class AdminModel extends BaseModel
{
    ...
    {
        return json_encode(['id' => $admin['id']]);
    }
    /**
     * 获取登录后的用户Model
    ...
    {
        return AdminActiveRecord::query()->where('username', '=', $username)->where('status', '=', self::ADMIN_STATUS_ENABLE)->first();
    }
    /**
     * 校验密码
    ...
    public function getAdminList(array $condition = [])
    {
        ...
        $model = $model->andFilterWhere('username', 'like', '%' . $condition['username'] . '%')->andFilterWhere('id', '=', $condition['id'])->orderBy('create_time');
    ...
    {
        $fields = ['id', 'username', 'status', 'create_time', 'update_time', 'access_token', 'access_token_expire_time'];

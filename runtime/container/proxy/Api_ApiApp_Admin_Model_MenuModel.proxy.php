...
class MenuModel extends BaseModel
{
    ...
    {
        $query = MenuActiveRecord::select(['id', 'name', 'parent_id'])->where('name', 'like', '%' . $condition['name'] . '%');
        $count = $query->count('id');
        $query = $query->offset($condition['offset'])->limit($condition['limit']);
        $menus = $query->get()->toArray();
        return ['count' => $count, 'parentMenus' => $menus];
    }
    /**
     * 获取菜单列表
     * @param array $condition
    ...
    {
        $query = MenuActiveRecord::query()->where('name', 'like', '%' . $condition['name'] . '%')->orderBy('create_time');
        if ($condition['isParent']) {
            $query = $query->where('parent_id', '=', self::DEFAULT_PARENT_ID);
        }
        $count = $query->count('id');
        $query = $query->offset($condition['offset'])->limit($condition['limit']);
        return ['count' => $count, 'list' => $query->get()->toArray()];
    }
    /**
     * 删除单个菜单
     * @param array $condition
     ...
     * @return int
     */
    public function updateMenu(int $adminId, array $data = [])
    {
        Db::beginTransaction();
        try {
            //更新菜单数排序据
            ...
            //更新管理员菜单数据[排序]
            AdminMenuActiveRecord::where('menu_id', '=', $data['id'])->where('admin_id', '=', $adminId)->update(['sort' => $data['sort']]);
            Db::commit();
            return true;
        } catch (\Exception $e) {
            Db::rollBack();
        ...
        return MenuActiveRecord::where('id', $data['id'])->update($data);
    }
    /**
     * @return string
     */

...
class PermissionController extends BaseController
{
    ...
    private function createPermission()
    {
        ...
        foreach ($functions as $k => $v) {
            ...
            $createData[] = ['api_app' => $this->apiApp, 'api_version' => $this->apiVersion, 'api_class' => $class, 'api_function' => $function, 'api_route' => $route];
        }
        ...
        foreach ($namespaces as $namespace) {
            try {
                ...
                foreach ($annotationClasses as $annotationClass) {
                    ...
                    if (!is_object($annotationClass) || property_exists($annotationClass, 'prefix') !== true) {
                    ...
                    foreach ($methods as $method) {
                        ...
                        if (!is_object($annotationMethodObject) || property_exists($annotationMethodObject, 'path') !== true) {
    ...
    {
        $path = BASE_PATH . self::DS . 'api' . self::DS . 'ApiApp' . self::DS . $this->apiApp . self::DS . $this->apiVersion;
        return is_dir($path);
    }
    /**
     * 设置接口端入口

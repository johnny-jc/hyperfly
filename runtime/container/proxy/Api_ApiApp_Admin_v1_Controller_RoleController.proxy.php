...
class RoleController extends BaseController
{
    ...
    {
        $rules = ['id' => ['required', 'exists:' . $this->roleModel->getTable() . ',id'], 'name' => ['required', Rule::unique($this->roleModel->getTable())->ignore($this->request->input('id'))]];
        $message = ['id.required' => '参数:attribute不存在', 'id.exists' => '参数:attribute的值不存在', 'name.required' => '参数:attribute不存在', 'name.unique' => '参数:attribute的值已被占用'];
        $validate = $this->validator($this->request->all(), $rules, $message);
        if ($validate->fails()) {
            return $this->fail($validate->errors()->first());
        }
        ...
        return $this->roleModel->updateRoleById($roleId, $data) ? $this->success() : $this->fail();
    }
    /**
     * 更新角色
    ...
    {
        $rules = ['id' => ['required', 'exists:' . $this->roleModel->getTable() . ',id']];
        ...
        $roleId = (int) $this->request->input('id');
        return $this->roleModel->deleteRoleById($roleId) ? $this->success() : $this->fail();
    }
}
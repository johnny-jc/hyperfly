...
class AdminController extends ApiBaseController
{
    ...
    public function login(RequestInterface $request)
    {
        ...
        return $login ? $this->success(['user' => $login]) : $this->fail('账号或密码错误');
    }
    /**
     * @param RequestInterface $request
    ...
    {
        return $this->rbacAdminModel->logout((string) $request->input('admin_access_token')) ? $this->success() : $this->fail();
    }
    /**
    ...
    {
        $data = ['username' => 'super_admin', 'password' => password_hash('123456', PASSWORD_DEFAULT), 'status' => 1, 'access_token' => md5('auth'), 'access_token_expire_time' => date('Y-m-d H:i:s', time() + 7 * 24 * 60 * 60)];

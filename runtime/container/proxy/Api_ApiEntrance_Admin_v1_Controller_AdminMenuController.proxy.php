...
use Api\ApiEntrance\Admin\Model\AdminMenuModel;
...
class AdminMenuController extends BaseController
{
    ...
    private function formatInsertMenuData(int $adminId, array $menuArr)
    {
        ...
        foreach ($menuArr as $k => $v) {
            $v = explode('|', $v);
            $tmp = ['admin_id' => $adminId, 'menu_id' => $v[0], 'path' => $v[1], 'level' => $v[2]];
            $arr[] = $tmp;
        }
        return $arr;
    }
    /**
     * 登录
     * @RequestMapping(path="getAdminMenu", method="post")
     * @return mixed|\Psr\Http\Message\ResponseInterface
     */
    ...
    private function getTreeMenu(array $menu, int $parentId, array $adminMenuUnique)
    {
        ...
        foreach ($menu as $k => $v) {
            if ($v['parent_id'] == $parentId) {
                ...
                $tmp['title'] = $v['name'];
                $tmp['id'] = $v['id'];
                $tmp['basicData'] = ['path' => $v['path'], 'level' => $v['level']];
